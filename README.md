...
In order to use a submodule as an isolated project, use the script `detach.py` to detach it from the parent project. The script will place the project at the home directory of the current user and this project can then be built as a standalone maven project. Additional dependencies may need to be added to build the detached submodule.
